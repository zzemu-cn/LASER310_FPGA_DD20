// g++ -std=c++17 -o upload_dsk upload_dsk.cpp UART.cpp -mconsole -DMINGW_HAS_SECURE_API -D_FILE_OFFSET_BITS=64 -IC:/MinGW/include
#include <cstdint>
...
//    updateAvailablePortsInfo(ports);                // Setup/load time, the function will update the living struct
    ...
    if(argc<3) {
		fprintf(stderr, "usage: upload_dsk COMx filename.dsk\n");
		return 1;
	}
	...
	int err = uart_upload (argv[2], uart_com_no);
